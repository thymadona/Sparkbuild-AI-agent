...
| `lib/db/schemas/<table>.ts`                    | One table per file. Better Auth tables (`users`, `sessions`, `accounts`, `verifications`) use timestamp `mode: 'date'`; all 17 app tables use `mode: 'string'`. |
...
| `drizzle/`                                     | Migrations `0000_baseline` … `0009_drop_user_build_mode`, `meta/` snapshots, `_archive/` (pre-cutover history, not runnable), `README.md`.                      |
