...
export type LessonStep =
  | { kind: 'choose'; prompt: string; code?: string; options: string[]; answer: number; explain: string }
  ...
  | { kind: 'learn'; prompt: string; frames: { code: string; note: string; hl?: string; speak?: string }[] }
  ...
  | { kind: 'stage'; scene: 'room' | 'grid' | 'boxes' | 'machine'; prompt: string; config?: Record<string, unknown>; goal: Record<string, unknown>; palette: { label: string; ops: string[] }[]; solution: number[] }
