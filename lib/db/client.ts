...
const toIso = (x: string) => new Date(/[+-]\d{2}(:?\d{2})?$|Z$/.test(x) ? x : x + 'Z').toISOString()
...
function withOurParsers<T extends { types?: unknown }>(config: T): T {
  const inherited = (config.types as { getTypeParser?: (o: number, f?: unknown) => unknown })
    ?.getTypeParser
  return {
    ...config,
    types: {
      getTypeParser: (oid: number, format?: unknown) =>
        PARSERS[oid] ?? inherited?.(oid, format) ?? pg.types.getTypeParser(oid, format as never),
    },
  }
}
...
if (!isTest) {
  try {
    ...
    console.log(`[db] ${u.hostname}:${u.port || '5432'}${u.pathname} pool_max=${isProd ? 5 : 1}`)
  } catch {
