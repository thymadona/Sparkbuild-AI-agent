...
async function readPlayerStats(userId: string): Promise<PlayerStats> {
  const [rows, days] = await Promise.all([
    ...
    db.select({ day: activityDays.day }).from(activityDays).where(eq(activityDays.userId, userId)).orderBy(desc(activityDays.day)).limit(400),
  ])
  const xp = xpFor(rows)
  return {
    xp,
    level: levelFor(xp),
    streak: streakFor(days.map((d) => d.day), todayISO()),
