...
import { awaitingEditor, nextStepIndex, stepAction, stepNodeId, taskCodeNodeId, taskPageId } from '@/lib/board/tasks'
...
export interface Program { file: string; source: string; stdout: string | null; error: string | null; stale: boolean; ran?: boolean }
...
export function taskPrograms(
  ...
  for (const id of page?.nodeIds ?? []) {
    ...
    const output = page?.nodeIds.map((i) => board.nodes[i]).reverse().find((o) => o?.type === 'output' && o.forNodeId === id)
    const ran = run?.nodeId === id
    // Outputs written before `ran` existed carry no source; they count as fresh.
    const stale = !ran && output?.type === 'output' && output.ran !== undefined && output.ran !== n.source
    out.push({
      file: fileOf(n, entry),
      ...
      stale,
      ...(ran ? { ran } : {}),
      error: ran ? run.stderr.trim().split('\n').slice(-3).join('\n') || null : output?.type === 'output' && output.stderr ? output.stderr.trim().split('\n').slice(-3).join('\n') : null,
    })
  }
  return out
}
...
export function outputVerdict(check: TaskCheck, programs: Program[], entry: string): boolean | undefined {
...
export function describeTaskState(board: BoardState, task: LessonTask, programs: Program[], entry = 'main.py'): string {
  ...
  if (awaitingEditor(board, task, taskPageId(task))) {
    ...
    lines.push(describeStep(board, task) ?? `- Scripted concept steps, about ${shown} of ${task.steps?.length ?? 0}.`)
  ...
  if (task.then) {
    lines.push(board.nodes[`${taskCodeNodeId(task)}_2`]
  ...
  const started = [task.starter && `main program started as:\n${task.starter}`, task.then && `${task.then.file} started as:\n${task.then.source}`].filter(Boolean)
  if (started.length) lines.push(`- What the student was given (their words must differ from these):\n${started.join('\n')}`)
  const code = programs[0]?.source ?? ''
  for (const c of task.checks ?? []) {
    const v = outputVerdict(c, programs, entry)
    const state = v !== undefined ? (v ? 'met (checked against the run)' : 'NOT met') : isRuntimeCheck(c) ? 'judge from the output' : runTaskChecks([c], code)[0].passed ? 'met' : 'not met'
