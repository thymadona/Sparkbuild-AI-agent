...
export const ClientEvent = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('step_answer'), nodeId: NodeId, prompt: z.string().max(400), picked: z.string().max(160), attempts: z.number().int().min(0).max(20) }),
  z.object({ type: z.literal('stage_result'), nodeId: NodeId, prompt: z.string().max(400), program: z.array(z.string().max(40)).max(8), attempts: z.number().int().min(0).max(20) }),
])
...
export function applyClientEvent(board: BoardState, ev: ClientEvent): { board: BoardState; content: string; saveText?: string } {
  switch (ev.type) {
    ...
    case 'code_run_result': {
      ...
      const codeIds = (page?.nodeIds ?? []).filter((i) => { const n = board.nodes[i]; return n?.type === 'code' && n.editable })
      const node = board.nodes[ev.nodeId]
      const which = { file: node?.type === 'code' ? node.file ?? null : null, program: `${codeIds.indexOf(ev.nodeId) + 1} of ${codeIds.length} on this page` }
      // The tutor gets the tail of stderr (where Python puts the real error), not the whole traceback.
      const payload = { nodeId: ev.nodeId, ...which, ok: ev.ok, stdout: ev.stdout.slice(0, 500), error: ev.stderr.trim().split('\n').slice(-3).join('\n') }
    ...
    case 'stage_result': {
      ...
      const note = ev.type === 'step_answer'
      ...
      return { board, content: `<student_event type="${type}">${JSON.stringify(payload)}</student_event>\n${note}` }
    }
    case 'trace_result': {
