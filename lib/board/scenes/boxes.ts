...
export interface BoxesConfig {
  boxes: { name: string; value?: string | number }[]
}
export interface BoxesGoal {
  values: Record<string, string | number>
}
export type BoxesState = Record<string, string | number | undefined>

export const initial = (c: BoxesConfig): BoxesState =>
  Object.fromEntries(c.boxes.map((b) => [b.name, b.value]))
...
export function apply(s: BoxesState, op: string): BoxesState {
  ...
  if (kind === 'add')
    return { ...s, [a]: (typeof s[a] === 'number' ? (s[a] as number) : 0) + Number(b) }
  return s
}

export const won = (s: BoxesState, g: BoxesGoal) =>
  Object.entries(g.values).every(([k, v]) => String(s[k]) === String(v))
