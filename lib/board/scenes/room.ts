...
export const view = (state: RoomState) => ({
  ...worldState(state.events),
  says: state.events.filter(([k]) => k === 'say').map(([, a]) => a),
})
...
export function won(state: RoomState, goal: RoomGoal): boolean {
  ...
  if (
    goal.says &&
    (v.says.length !== goal.says.length ||
      v.says.some((s, i) => s.toLowerCase() !== goal.says![i].toLowerCase()))
  )
    return false
