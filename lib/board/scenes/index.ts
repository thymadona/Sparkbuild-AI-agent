...
export interface Block { label: string; ops: string[] }
...
export const initialState = (scene: SceneId, config: Any): unknown => call(SCENES[scene].initial, config)

// Every state a program passes through: [start, after op 1, after op 2, ...].
export function runProgram(scene: SceneId, config: Any, palette: Block[], program: number[]): unknown[] {
  const s = SCENES[scene]
  const states = [initialState(scene, config)]
  for (const i of program) for (const op of palette[i]?.ops ?? []) states.push(call(s.apply, states[states.length - 1], op, config))
  return states
}

export const sceneWon = (scene: SceneId, state: unknown, goal: Any) => call<boolean>(SCENES[scene].won, state, goal)
export const describeScene = (scene: SceneId, config: Any, goal: Any) => call<string>(SCENES[scene].describe, config, goal)
export const programWins = (scene: SceneId, config: Any, goal: Any, palette: Block[], program: number[]) =>
