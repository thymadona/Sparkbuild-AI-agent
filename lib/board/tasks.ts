...
export function taskFile(task: LessonTask, entry: string): string {
  for (const check of task.checks ?? []) if ('file' in check && check.file && check.file !== task.then?.file) return check.file
  return entry
}
...
export const stepNodeId = (task: LessonTask, i: number) => `step_${task.id.replace(/[^a-zA-Z0-9_]/g, '_')}_${i}`

export function stepNode(task: LessonTask, i: number): BoardNode | null {
  const step = task.steps?.[i]
  if (!step) return null
  const base = { id: stepNodeId(task, i), parentId: null, createdBy: 'system' as const, answered: false }
  if (step.kind === 'try') {
    return { ...base, type: 'sandbox', prompt: step.prompt, template: step.template, chips: step.chips ?? [], value: '', said: '', seen: [], need: step.need }
  }
  if (step.kind === 'learn') return { ...base, type: 'learn', prompt: step.prompt, frames: step.frames, frame: 0 }
  // Tiles start reversed (never the answer) and stay put across a reload.
  if (step.kind === 'order') return { ...base, type: 'order', prompt: step.prompt, lines: step.lines, arranged: [], attempts: 0 }
  if (step.kind === 'bug') return { ...base, type: 'bug', prompt: step.prompt, code: step.code, bugLine: step.bugLine, explain: step.explain, picked: null, attempts: 0 }
  if (step.kind === 'stage') return { ...base, type: 'stage', prompt: step.prompt, scene: step.scene, config: step.config ?? {}, goal: step.goal, palette: step.palette, solution: step.solution, program: [], attempts: 0 }
  if (step.kind === 'match') return { ...base, type: 'match', prompt: step.prompt, pairs: step.pairs, matched: [], picked: null, attempts: 0 }
  return {
    ...base,
    type: 'quiz',
...
const CHEERS = ['Great job!', 'Good job!', 'Nice one!', 'Awesome!', 'Well done!', 'You got it!', 'Super!', 'Way to go!']
...
export function stepAction(node: BoardNode): string {
  switch (node.type) {
    case 'learn': return node.frame >= node.frames.length - 1 ? 'Tap Got it when you are ready.' : 'Tap Next to keep going.'
    case 'sandbox': return node.chips.length ? 'Tap a line, then press Say it.' : 'Type your words. Then press Say it.'
    case 'quiz': return 'Tap the answer you think is right.'
    case 'order': return 'Tap the lines in order. Then press Say it.'
    case 'bug': return 'Tap the line with the mistake.'
    case 'match': return 'Tap a code piece. Then tap what it does.'
    case 'stage': return 'Tap blocks to build a program. Then press Run.'
    default: return ''
  }
}
...
export function awaitingEditor(board: BoardState, task: LessonTask | null, pageId: string | null): boolean {
