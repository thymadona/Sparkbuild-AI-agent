...
export function taskCode(board: BoardState | null, files: Record<string, string>, entry: string, task: LessonTask): string {
  if (board) {
 ...
 */
export function verifyTask(task: LessonTask, code: string, reported: RuntimeVerdicts = []): Verdict {
  const verdicts = (task.checks ?? []).map((check, i) => (isRuntimeCheck(check) ? reported[i] : undefined))
  const results = runTaskChecks(task.checks, code, verdicts)
  return { results, passed: results.every((r) => r.passed), failed: results.find((r) => !r.passed) ?? null }
}
