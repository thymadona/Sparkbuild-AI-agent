 ...
 */
export async function recordTaskDone(projectId: string, userId: string, taskId: string, judged: Judgement): Promise<string[]> {
  ...
  const { ids, fresh } = await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .onConflictDoUpdate({ target: lessonProgress.projectId, set: { completedTaskIds: next, updatedAt: now } })
    await tx
      .insert(taskProgress)
      .values({
        projectId,
        ...
        output: Object.fromEntries(judged.programs.filter((p) => p.stdout !== null).map((p) => [p.file, p.stdout])),
      })
      .onConflictDoNothing()
