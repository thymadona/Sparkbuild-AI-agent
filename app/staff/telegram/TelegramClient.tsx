  ...
  return (
    <div className="space-y-5">
      <Card>
        <CardContent className="space-y-3">
          ...
          <ol className="space-y-1.5 text-sm text-muted-foreground list-decimal list-inside">
            <li>Create your bot via <span className="text-foreground">@BotFather</span> and set <code className="rounded bg-muted px-1 text-primary">TELEGRAM_BOT_TOKEN</code></li>
            <li>Share the bot link with the parent</li>
            <li>Parent opens Telegram and sends <span className="text-foreground">/start</span> to the bot</li>
            <li>Click <span className="text-foreground">Fetch Recent Chats</span> below to see their chat ID</li>
                  ...
                  <TableCell>
                    ...
                    >
                      ...
                      {copied === c.chat_id ? (
                        ...
                        <svg className="w-3 h-3 text-muted-foreground/70" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
      ...
      {chats.length === 0 && !loading && !error && (
        <p className="text-sm text-muted-foreground/70">No chats yet. Fetch after a parent sends /start to your bot.</p>
      )}
    </div>
  )
}
