...
export default function DashboardClient({
  ...
  const lessonsStarted = new Set(
    projects.filter((p) => LESSONS.some((l) => l.id === p.lesson_id)).map((p) => p.lesson_id)
  ).size
        ...
        <div className="space-y-10">
          ...
          <section>
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link
                href="/lessons"
                className="inline-flex items-center gap-1 text-sm font-semibold text-fg-secondary transition-colors hover:text-fg-primary"
              >
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {LESSONS.slice(0, 3).map((lesson, i) => {
                ...
                return (
                  <div
                    key={lesson.id}
                    className={`rounded-2xl border border-border bg-card p-5 ${locked ? 'opacity-60' : ''}`}
                  >
                    <div
                      className={`flex h-11 w-11 items-center justify-center rounded-full ${MODULE_CHIPS[i] ?? 'bg-tint-sage'}`}
                    >
                      {locked ? (
                        <Lock className="h-5 w-5 text-fg-primary" />
                      ) : (
                    ...
                    <div className="mt-4">
                      {locked ? (
                        <span className="inline-block rounded-full bg-muted px-4 py-2 text-sm font-semibold text-fg-muted">
                          Not open yet
                        </span>
                      ) : (
                        <Link href="/lessons" className="btn-outline">
        ...
        {projects.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border py-16 text-center">
            ...
            <p className="mb-6 mt-2 text-sm text-fg-muted">
              Start a lesson to make your first project.
            </p>
            ...
            {projects.map((project) => {
              ...
              return (
                <div key={project.id} className="rounded-2xl border border-border bg-card p-5">
                  ...
                  <p className="mb-0.5 truncate text-sm font-semibold text-fg-primary">
                    {project.title}
                  </p>
                  <p className="mb-4 text-xs text-fg-muted">
                    {weekIndex >= 0 ? `Week ${weekIndex + 1}` : 'Older course'}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-xs">
                    ...
                    <button
                      ...
                      className="btn-outline ml-auto !px-3 !py-1.5 !text-xs !text-red-600 hover:!bg-red-50"
                    >
                      Delete
                    </button>
                  </div>
