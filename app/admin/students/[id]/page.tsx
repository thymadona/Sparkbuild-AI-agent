  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Link href="/admin/students" className="hover:text-gray-300 transition-colors">Students</Link>
        <span>/</span>
          ...
          <div className="divide-y divide-gray-800">
            {enrolledClasses.map((cls) => (
              <div key={cls.id} className="px-5 py-3 flex items-start justify-between gap-4">
                ...
                <Link href={`/admin/classes/${cls.id}`} className="text-xs text-gray-600 hover:text-gray-400 shrink-0">
                  Details →
                </Link>
              </div>
            ))}
