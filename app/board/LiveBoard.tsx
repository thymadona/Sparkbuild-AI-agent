...
import { awaitingEditor, isTaskOpen, nextStepIndex, cheer, stepAction, stepNode, stepNodeId, taskCodeNodeId, taskFile, taskForPageId, taskIndexForPageId, taskPageId, taskStarter } from '@/lib/board/tasks'
...
export default function LiveBoard({ projectId, initialBoard, lastCaption, lesson, entry, files, completedTaskIds, submission, classSlots }: Props) {
  ...
  const onTaskDone = useCallback((_taskId: string, ids: string[]) => progressRef.current.applyDone(ids), [])
  const { captions, live, mascot, busy, send, say } = useTutor(projectId, dispatch, lastCaption ? [lastCaption] : [], onTrace, undefined, onTaskDone)
  ...
  useEffect(() => {
    ...
    try { setVoice(localStorage.getItem('spark-voice') === '1') } catch { /* private window: stays off */ }
    return () => stopSpeaking()
  }, [])
  const toggleVoice = useCallback(() => {
    setVoice((on) => {
      try { localStorage.setItem('spark-voice', on ? '0' : '1') } catch { /* not remembered */ }
      if (on) stopSpeaking()
      return !on
    })
  }, [])
  ...
  const checkFiles = useMemo(() => ({ ...files, ...boardFiles(board, entry), ...(ownEntry ? { [entry]: code } : {}) }), [files, board, entry, ownEntry, code])
  const runtimeChecks = useRuntimeChecks(waiting ? undefined : viewedTask ?? undefined, checkFiles, entry)
  ...
  const addStep = useCallback((task: LessonTask, i: number) => {
    ...
    if ('prompt' in node) say(`${i > 0 ? `${cheer(i)} Now: ` : ''}${node.prompt} ${stepAction(node)}`.trim())
  }, [say])
  const addEditor = useCallback((task: LessonTask, prevSource?: string) => {
    ...
    if (task.go && !opened.current.has(goId) && !b.nodes[goId]) {
      ...
      dispatch({ op: { op: 'add', pageId: taskPageId(task), node: { id: goId, parentId: null, createdBy: 'system', type: 'text', markdown: `**${task.go}**` } }, actor: 'client' })
    }
    ...
    const source = own ?? ((file === entry ? prevSource ?? carry.current[task.id] ?? boardCode(b) ?? files[entry] : files[file]) ?? '')
    dispatch({
      op: {
        op: 'add',
        pageId: taskPageId(task),
        node: { id: nodeId, parentId: null, createdBy: 'student', type: 'code', language: 'python', file: file === entry ? undefined : file, anchor: own === null ? task.commentAnchor : undefined, source, editable: true },
      },
      actor: 'client',
    })
  }, [entry, files])
  ...
  useEffect(() => {
    ...
    if (next !== null) { addStep(task, next); return }
    const last = stepNodeId(task, task.steps.length - 1)
    const lastNode = board.nodes[last]
    if (lastNode && 'answered' in lastNode && lastNode.answered && !opened.current.has(taskCodeNodeId(task))) {
      addEditor(task)
  ...
  useEffect(() => {
    ...
    if (revealed.current.has(codeId) || board.nodes[codeId] || !checks.results[then.after]?.passed) return
    ...
    const pageId = taskPageId(task)
    dispatch({ op: { op: 'add', pageId, node: { id: `go_${codeId}`, parentId: null, createdBy: 'system', type: 'text', markdown: `**${then.go}**` } }, actor: 'client' })
    dispatch({
      op: { op: 'add', pageId, node: { id: codeId, parentId: null, createdBy: 'student', type: 'code', language: 'python', file: then.file, source: then.source, editable: true } },
      actor: 'client',
    })
  ...
  useEffect(() => {
    ...
    void saveBoard(boardRef.current).then(() => send({ type: 'code_run_result', nodeId: id, ...result }))
  ...
  const codeActions: CodeActions = {
    ...
    feedback: (event) => { setTimeout(() => { void saveBoard(boardRef.current).then(() => send(event)) }, 120) },
  }
