      ...
      <pre className="p-4 text-sm leading-6 font-mono overflow-x-auto">
        {node.source.split('\n').map((line, i) => (
          <div
            ...
            className={cn(
              'px-2 -mx-2 rounded',
              node.highlightLines.includes(i + 1) && 'bg-amber-400/25'
            )}
          >
    ...
    <figure className="rounded-xl overflow-hidden border border-[#3b2a1c]">
      ...
      <div className="flex items-center gap-3 bg-[#2b2118] px-3 py-2">
        {running ? (
          ...
          <button
            onClick={() => code.run(node, view.current?.state.doc.toString() ?? node.source)}
    ...
    <div className="rounded-xl border border-[#e4d9c5] p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {node.options?.map((o) => (
          <span
            key={o}
            className="min-h-11 inline-flex items-center rounded-full border border-[#d9c9ab] px-4 text-sm"
          >
            {o}
          </span>
        ))}
