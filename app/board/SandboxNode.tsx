    ...
    <div className="rounded-xl border border-[#e4d9c5] p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {node.chips.map((chip) => {
            ...
            return (
              ...
              >
                {tried && <span aria-hidden="true">✓ </span>}{node.template.replace('{}', chip)}
              </button>
            )
          })}
        </div>
      )}
      <form className="flex flex-wrap items-center gap-2" onSubmit={(e) => { e.preventDefault(); say(text) }}>
        ...
        <button disabled={!text.trim() || !code} className="min-h-11 rounded-full bg-emerald-500 px-5 text-sm font-bold text-white disabled:opacity-50">▶ Say it</button>
      </form>
      ...
      {node.need > 1 && (
        <p role="status" className="text-sm font-semibold text-[#7a4a10]">
          {node.answered ? 'Great! You did it.' : node.seen.length === 0 ? `Click ${node.need} different lines.` : `Good! Click ${node.need - node.seen.length} more ${node.need - node.seen.length === 1 ? "line" : "lines"}. ${node.seen.length} of ${node.need} done.`}
        </p>
      )}
    </div>
  )
