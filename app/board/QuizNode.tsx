...
export default function QuizNode({
  node,
  code,
}: {
  node: Extract<BoardNode, { type: 'quiz' }>
  code?: CodeActions
}) {
  ...
  return (
    <div className="rounded-xl border border-[#e4d9c5] p-4 space-y-3">
      ...
      {node.code && (
        <pre className="rounded-lg bg-[#2b2118] p-3 font-mono text-sm text-[#f3e9d8] overflow-x-auto">
          {node.code}
        </pre>
      )}
      <div className="flex flex-wrap gap-2" role="group" aria-label={node.prompt}>
        {node.options?.map((o, i) => {
          ...
          return (
            ...
            >
              {shown === 'right' && <span aria-hidden="true">✓ </span>}
              {o}
            </button>
          )
        })}
