...
function SceneView({ node, state, frame }: { node: Stage; state: unknown; frame: number }) {
  switch (node.scene) {
    case 'room':
      return <RoomView state={state as RoomState} frame={frame} />
    case 'grid':
      return <GridView state={state as GridState} config={node.config as unknown as GridConfig} />
    case 'boxes':
      return (
        <BoxesView state={state as BoxesState} config={node.config as unknown as BoxesConfig} />
      )
    case 'machine':
      return (
        <MachineView
          state={state as MachineState}
          config={node.config as unknown as MachineConfig}
        />
      )
  }
}
...
export default function StageNode({ node, code }: { node: Stage; code?: CodeActions }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (sceneWon(scene, states.at(-1), node.goal))
        return code.patch(node.id, { attempts, answered: true })
      setMissed(true)
      code.feedback?.({
        type: 'stage_result',
        nodeId: node.id,
        prompt: node.prompt,
        program: node.program.map((i) => palette[i].label),
        attempts,
      })
      code.patch(
        node.id,
  ...
  const add = (i: number) => {
    setMissed(false)
    code?.patch(node.id, { program: [...node.program, i] })
  }
  const remove = (at: number) => {
    setMissed(false)
    code?.patch(node.id, { program: node.program.filter((_, k) => k !== at) })
  }
  const run = () => {
    setMissed(false)
    setPlaying(0)
  ...
  return (
    <div className="rounded-xl border border-[#e4d9c5] p-4 space-y-3">
      ...
      <ol
        className="min-h-11 space-y-1 rounded-lg border-2 border-dashed border-[#d9c9ab] p-2"
        aria-label="Your program"
      >
        {node.program.length === 0 && (
          <li className="px-2 py-1 text-sm text-[#5c4f3d]">Tap blocks to build your program.</li>
        )}
        {node.program.map((i, at) => (
          <li key={at} className="flex items-center gap-2">
            <span className="w-5 text-right text-sm text-[#5c4f3d]" aria-hidden="true">
              {at + 1}
            </span>
            <button
              disabled={locked}
              onClick={() => remove(at)}
              aria-label={`Remove ${palette[i].label}`}
              className="min-h-9 rounded-lg border-2 border-[#d9c9ab] bg-white px-3 font-mono text-sm font-semibold hover:bg-red-50 disabled:hover:bg-white"
            >
              {palette[i].label}
            </button>
          </li>
      ...
      {!node.answered && !revealed && (
        <div className="flex flex-wrap gap-2" role="group" aria-label="Blocks">
          {palette.map((b, i) => (
            <button
              key={i}
              disabled={locked || node.program.length >= MAX_BLOCKS}
              onClick={() => add(i)}
              className={cn(
                'min-h-11 rounded-lg border-2 border-[#2b2118] bg-[#2b2118] px-4 font-mono text-sm font-semibold text-[#f3e9d8] hover:bg-[#3b2a1c] focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-50'
              )}
            >
              {b.label}
            </button>
      ...
      {!node.answered && !revealed && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            onClick={() => {
              setMissed(false)
              code?.patch(node.id, { program: [] })
            }}
            className="min-h-11 rounded-full border-2 border-[#d9c9ab] px-4 text-sm font-semibold hover:bg-[#e4d3b3] disabled:opacity-50"
          >
            ↻ Start over
          </button>
        </div>
