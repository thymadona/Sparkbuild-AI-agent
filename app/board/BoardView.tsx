      ...
      <div className="flex h-full gap-3">
        ...
        <main className="relative flex-1 min-w-0 rounded-3xl bg-[#fffdf8] shadow-md">
          <div
            ref={paperRef}
            onScroll={onScroll}
            className="h-full overflow-y-auto px-6 md:px-12 py-8 pb-56"
          >
              ...
              <div className="space-y-5">
                {page?.nodeIds.map((id) => (
                  <div
                    key={id}
                    id={`node-${id}`}
                    className={cn('board-rise rounded-xl', board.focusId === id && 'board-pulse')}
                  >
                    <NodeView
                      node={board.nodes[id]}
                      code={code}
            ...
            {!minimized ? (
              <div className="pointer-events-auto min-w-[16rem] max-w-md flex-1 rounded-2xl bg-[#3b2a1c] p-4 text-[#faf6ee] shadow-lg">
                ...
                <div className="mt-2 flex gap-3 text-xs text-[#faf6ee]/80">
                  {captions.length > 1 && (
                    <button className="underline min-h-6" onClick={() => setShowEarlier((v) => !v)}>
                      {showEarlier ? 'Hide' : 'Show'} earlier captions
                    </button>
                  )}
                  <button className="underline min-h-6" onClick={() => setMinimized(true)}>
                    Minimize
                  </button>
                </div>
              </div>
              ...
              <button
                className="pointer-events-auto min-h-11 rounded-full bg-[#3b2a1c] px-4 text-sm text-[#faf6ee]"
            ...
            >
              ...
              {onReplay ? (
                <button
                  type="button"
                  onClick={onReplay}
                  className="min-h-11 rounded-full bg-amber-500 px-4 text-sm font-semibold text-[#2b2118]"
                >
                  Replay
                </button>
              ) : (
                <button
                  ...
                  aria-label="Send"
                  className="grid size-11 shrink-0 place-items-center rounded-full bg-[#2b2118] text-[#faf6ee] transition-colors disabled:bg-[#a29a8f]"
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="size-5"
