...
export async function POST(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const body = (await req.json().catch(() => ({}))) as { lessonId?: number; disabled?: boolean }
  const { lessonId, disabled } = body
  ...
  if (disabled) {
    const { error } = await supabaseAdmin
      ...
      .upsert({ class_id: params.id, lesson_id: lessonId, disabled_by: user.id }, { onConflict: 'class_id,lesson_id' })
