import { describeStep, describeTaskState, hasRun, outputVerdict, taskPrograms } from '@/lib/task-evidence'
...
const code = (id: string, source: string, file?: string) => ({ id, parentId: null, createdBy: 'student' as const, type: 'code' as const, language: 'python' as const, source, editable: true, file })
const boardWith = (source: string, ran?: string): BoardState => {
  ...
  return {
    ...
    nodes: {
      [id]: code(id, source),
      [`out_${id}`]: { id: `out_${id}`, parentId: null, createdBy: 'system', type: 'output', forNodeId: id, stdout: 'hi\n', stderr: '', ok: true, ran },
    },
...
describe('describeTaskState', () => {
  ...
  it('says there is no editor during the concept steps', () => {
    const b = { pages: [{ id: page, title: '', nodeIds: [] }], activePageId: page, focusId: null, nodes: {} } as BoardState
...
describe('code_run event', () => {
  it('names which of two programs ran', () => {
    ...
    const { content } = applyClientEvent(b, { type: 'code_run_result', nodeId: `${id}_2`, source: 'print("x")', ok: true, stdout: 'x\n', stderr: '' })
...
describe('outputVerdict', () => {
  const prog = (file: string, stdout: string | null, stale = false) => ({ file, source: '', stdout, error: null, stale })
...
describe('describeStep', () => {
  const withSteps = (n: number, patch: Record<string, unknown> = {}) => {
    ...
    const ids: string[] = []
    for (let i = 0; i < n; i++) {
      const node = { ...stepNode(task, i)!, ...(i === n - 1 ? patch : { answered: true }) } as { id: string }
      nodes[node.id] = node
      ids.push(node.id)
    }
    return { pages: [{ id: page, title: '', nodeIds: ids }], activePageId: page, focusId: null, nodes } as unknown as BoardState
  }
...
describe('step feedback events', () => {
  ...
  it('step_answer tells the tutor what was picked and to hold the answer back', () => {
    const { content } = applyClientEvent(board, { type: 'step_answer', nodeId: 'step_x_1', prompt: 'What will Sparky say?', picked: '"Hi Ada"', attempts: 1 })
  ...
  it('stage_result carries the program and needs no board node', () => {
    const { content } = applyClientEvent(board, { type: 'stage_result', nodeId: 'step_x_0', prompt: 'Get the gem', program: ['move'], attempts: 3 })
