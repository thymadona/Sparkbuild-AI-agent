...
import { boardCode, boardFiles, boardFromFiles, codeNodeId, pageCode, withBoardCode, SavedBoard } from '@/lib/board/code'
...
describe('board code', () => {
  ...
  it('scopes code to one page, so a later task cannot answer an earlier one', () => {
    ...
    const withFirst = apply(first, { op: 'add', pageId: 't_one', node: { id: 'c1', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print(1)', editable: true, highlightLines: [] } })
    const second = apply(withFirst, { op: 'new_page', pageId: 't_two', title: 'Two' })
    const board = apply(second, { op: 'add', pageId: 't_two', node: { id: 'c2', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print(2)', editable: true, highlightLines: [] } })
  ...
  it('writes each node back to its own file, defaulting to the entry', () => {
    ...
    const main = apply(base, { op: 'add', pageId: 'p1', node: { id: 'c1', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print(1)', editable: true, highlightLines: [] } })
    const board = apply(main, { op: 'add', pageId: 'p1', node: { id: 'c2', parentId: null, createdBy: 'student', type: 'code', language: 'python', file: 'bugzap.py', source: 'fixed', editable: true, highlightLines: [] } })
