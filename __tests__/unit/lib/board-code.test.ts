...
import { blockOf, boardCode, boardFiles, boardFromFiles, codeNodeId, pageCode, withBoardCode, SavedBoard } from '@/lib/board/code'
...
const withCode = (b: ReturnType<typeof emptyBoard>, id: string, source: string) => {
  ...
  return apply(base, { op: 'add', pageId: base.activePageId ?? 'p1', node: { id, parentId: null, createdBy: 'tutor', type: 'code', language: 'python', source, editable: true } })
}
...
describe('board code', () => {
  ...
  it('scopes code to one page, so a later task cannot answer an earlier one', () => {
    ...
    const withFirst = apply(first, { op: 'add', pageId: 't_one', node: { id: 'c1', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print(1)', editable: true } })
    const second = apply(withFirst, { op: 'new_page', pageId: 't_two', title: 'Two' })
    const board = apply(second, { op: 'add', pageId: 't_two', node: { id: 'c2', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print(2)', editable: true } })
  ...
  it('writes each node back to its own file, defaulting to the entry', () => {
    ...
    const main = apply(base, { op: 'add', pageId: 'p1', node: { id: 'c1', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print(1)', editable: true } })
    const board = apply(main, { op: 'add', pageId: 'p1', node: { id: 'c2', parentId: null, createdBy: 'student', type: 'code', language: 'python', file: 'bugzap.py', source: 'fixed', editable: true } })

    expect(boardFiles(board, 'main.py')).toEqual({ 'main.py': 'print(1)', 'bugzap.py': 'fixed' })
    expect(withBoardCode({ 'main.py': 'old', 'bugzap.py': 'broken' }, 'main.py', board)).toEqual({ 'main.py': 'print(1)', 'bugzap.py': 'fixed' })
  })
...
describe('blockOf (the part of the file a task shows)', () => {
  ...
  it('puts an edit back without touching the rest of the file', () => {
    const b = blockOf(file, 'TASK: first-words')
    const edited = b.compose('# Change the words inside the quotes.\nprint("hello")\nprint("again")')
    expect(edited.split('\n').slice(0, 3).join('\n')).toBe(file.split('\n').slice(0, 3).join('\n'))
  ...
  it('never mistakes a task\'s second program for the main one', () => {
    ...
    const node = (id: string, source: string, file?: string) => ({ id, parentId: null, createdBy: 'student', type: 'code', language: 'python', source, editable: true, ...(file ? { file } : {}) })
    ...
    expect(boardFiles(b, 'main.py')).toEqual({ 'main.py': 'print("main")', 'line2.py': 'print("side")' })
  })
})
