...
import { cheer, stepAction, awaitingEditor, isTaskOpen, nextStepIndex, stepNode, stepNodeId, taskCodeNodeId, taskFile, taskForPageId, taskIndexForPageId, taskPageId, taskStarter } from '@/lib/board/tasks'
...
describe('concept steps', () => {
  ...
  it('turns every step into a valid board node with a legal id', () => {
    for (const l of LESSONS) for (const task of l.tasks) for (let i = 0; i < (task.steps?.length ?? 0); i++) {
  ...
  it('has a correct answer inside the options of every choose step', () => {
    for (const l of LESSONS) for (const task of l.tasks) for (const step of task.steps ?? []) {
  ...
  it('tells the student what to do once the steps end', () => {
    for (const l of LESSONS) for (const task of l.tasks) if (task.steps?.length) expect(task.go).toBeTruthy()
  })
  ...
  it('has no editor to judge until the steps are done', () => {
    ...
    const editor: BoardNode = { id: taskCodeNodeId(first), parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print("x")', editable: true }
...
describe('taskStarter', () => {
  const code = (source: string) => ({ id: 'c', parentId: null, createdBy: 'student' as const, type: 'code' as const, language: 'python' as const, source, editable: true })
  const boardOf = (pageId: string, source: string) => ({ pages: [{ id: pageId, title: '', nodeIds: ['c'] }], activePageId: pageId, focusId: null, nodes: { c: code(source) } })
  const task = (over: object) => ({ id: 'x', type: 'core', chip: '', success: '', prompt: '', ...over }) as never
