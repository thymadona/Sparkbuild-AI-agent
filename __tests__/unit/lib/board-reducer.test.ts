...
describe('board reducer', () => {
  ...
  describe('lesson steps', () => {
    const quiz = { id: 'q1', parentId: null, createdBy: 'system', type: 'quiz', kind: 'multiple_choice', prompt: 'Which?', options: ['a', 'b'], answer: 1, picked: null, attempts: 0, answered: false }
    const boardWithQuiz = () => apply(withPage(), { op: 'add', pageId: 'p1', node: quiz }, 'client')

    it('lets the client record an answer', () => {
      const s = apply(boardWithQuiz(), { op: 'update', id: 'q1', patch: { picked: 1, attempts: 1, answered: true } }, 'client')
    ...
    it('stops the tutor answering a step for the student', () => {
      ...
      for (const patch of [{ answered: true }, { picked: 1 }, { seen: ['x'] }]) {
        expect(() => apply(boardWithQuiz(), { op: 'update', id: 'q1', patch }, 'tutor')).toThrow(/cannot change/)
      }
      expect(apply(boardWithQuiz(), { op: 'update', id: 'q1', patch: { prompt: 'Which one?' } }, 'tutor').nodes.q1).toMatchObject({ prompt: 'Which one?' })
    })

    it('keeps sandbox nodes away from the tutor', () => {
      const sandbox = { id: 's1', parentId: null, createdBy: 'tutor', type: 'sandbox', prompt: 'Try', template: 'print("{}")' }
      expect(() => apply(withPage(), { op: 'add', pageId: 'p1', node: sandbox }, 'tutor')).toThrow(/cannot create sandbox/)
    ...
    it('keeps the new step nodes and their state away from the tutor', () => {
      const shapes = {
        stage: { prompt: 'P', scene: 'room', palette: [{ label: 'a', ops: ['say:a'] }], solution: [0] },
        learn: { prompt: 'P', frames: [{ code: 'c', say: 's' }] },
        order: { prompt: 'P', lines: ['a', 'b'] },
        bug: { prompt: 'P', code: 'a', bugLine: 0, explain: 'e' },
        match: { prompt: 'P', pairs: [{ left: 'a', right: 'b' }, { left: 'c', right: 'd' }] },
      }
      for (const [type, shape] of Object.entries(shapes)) {
        expect(() => apply(withPage(), { op: 'add', pageId: 'p1', node: { id: 'x1', parentId: null, createdBy: 'tutor', type, ...shape } }, 'tutor')).toThrow(new RegExp(`cannot create ${type}`))
        expect(JSON.stringify(toolsFor(true))).not.toContain(`"${type}"`)
      }
      for (const field of ['frame', 'arranged', 'matched', 'program']) {
        expect(() => apply(boardWithQuiz(), { op: 'update', id: 'q1', patch: { [field]: 1 } }, 'tutor')).toThrow(/cannot change/)
      }
    })

    it('still loads a quiz saved before steps existed', () => {
      const old = { id: 'q0', parentId: null, createdBy: 'tutor', type: 'quiz', kind: 'multiple_choice', prompt: 'Old?', options: ['a'] }
      expect(apply(withPage(), { op: 'add', pageId: 'p1', node: old }, 'tutor').nodes.q0).toMatchObject({ attempts: 0, answered: false })
    })
  })
})
