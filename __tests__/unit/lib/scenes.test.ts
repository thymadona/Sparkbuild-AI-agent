...
const stages = PY_LESSONS.flatMap((l) =>
  l.tasks.flatMap((t) =>
    (t.steps ?? []).flatMap((s) =>
      s.kind === 'stage' ? [{ where: `${l.id}/${t.id}`, step: s }] : []
    )
  )
)
...
describe('stage steps in the catalog', () => {
  it('has at least one stage of each scene', () => {
    expect(new Set(stages.map((s) => s.step.scene))).toEqual(
      new Set(['room', 'grid', 'boxes', 'machine'])
    )
  })

  it.each(stages.map((s) => [s.where + ' ' + s.step.scene, s.step] as const))(
    '%s: the solution wins and an empty program does not',
    (_name, step) => {
