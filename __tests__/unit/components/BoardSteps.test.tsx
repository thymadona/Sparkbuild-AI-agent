...
const quiz: Quiz = { id: 'q', parentId: null, createdBy: 'system', type: 'quiz', kind: 'multiple_choice', prompt: 'Which?', options: ['no', 'yes', 'nope'], answer: 1, explain: 'Because yes.', picked: null, attempts: 0, answered: false }
const sandbox: Sandbox = { id: 's', parentId: null, createdBy: 'system', type: 'sandbox', prompt: 'Try', template: 'print("{}")', chips: ['hello', 'hi'], value: '', said: '', seen: [], need: 2, answered: false }

// The board owns the node; this stands in for its `patch` action.
function Harness<T extends Quiz | Sandbox>({ start, View }: { start: T; View: (p: { node: T; code: CodeActions }) => React.ReactNode }) {
  const [node, setNode] = useState(start)
  const code = { patch: (_id: string, patch: Record<string, unknown>) => setNode((n) => ({ ...n, ...patch }) as T) } as CodeActions
  return <>{View({ node, code })}</>
}

describe('quiz step', () => {
  const view = () => render(<Harness start={quiz} View={({ node, code }) => <QuizNode node={node} code={code} />} />)
...
describe('sandbox step', () => {
  it('needs distinct tries, then says the words', () => {
    render(<Harness start={sandbox} View={({ node, code }) => <SandboxNode node={node} code={code} />} />)
  ...
  it('says typed words and ignores blanks', () => {
    render(<Harness start={{ ...sandbox, chips: [], need: 1 }} View={({ node, code }) => <SandboxNode node={node} code={code} />} />)
...
const at = <T extends BoardNode['type']>(node: Record<string, unknown> & { type: T }) => ({ id: 'n', parentId: null, createdBy: 'system', answered: false, ...node }) as Extract<BoardNode, { type: T }>
const wrap = <T extends BoardNode>(start: T, View: (p: { node: T; code: CodeActions }) => React.ReactNode) => render(<Harness start={start as never} View={View as never} />)

describe('learn step', () => {
  const frames = [{ code: 'print', note: 'one', hl: 'print' }, { code: 'print("a")', note: 'two', hl: 'a', speak: 'a' }]
  const view = (f: unknown[] = frames) => wrap(at({ type: 'learn', prompt: 'Meet', frames: f, frame: 0 }), ({ node, code }) => <LearnNode node={node} code={code} />)
...
describe('order step', () => {
  const node = at({ type: 'order', prompt: 'Order', lines: ['print("Hi")', 'print("Bye")'], arranged: [], attempts: 0 })
  ...
  const play = (ms: number) => act(() => { jest.advanceTimersByTime(ms) })
...
describe('bug step', () => {
  const view = () => wrap(at({ type: 'bug', prompt: 'Tap', code: 'print("a")\nprint(b)', bugLine: 1, explain: 'Quotes.', picked: null, attempts: 0 }), ({ node, code }) => <BugNode node={node} code={code} />)
...
describe('match step', () => {
  it('locks pairs and resolves when all match, ignoring a wrong pair', () => {
    wrap(at({ type: 'match', prompt: 'Match', pairs: [{ left: 'print', right: 'speaks' }, { left: '"hi"', right: 'the words' }], matched: [], picked: null, attempts: 0 }), ({ node, code }) => <MatchNode node={node} code={code} />)
...
describe('feedback to the tutor', () => {
  ...
  const tracked = <T extends BoardNode>(start: T, View: (p: { node: T; code: CodeActions }) => React.ReactNode) =>
    render(<Harness start={start as never} View={({ node, code }) => View({ node: node as T, code: { ...code, feedback } as CodeActions })} />)
  ...
  it('quiz: a wrong pick reports the question, what was picked and the miss count', () => {
    ...
    fireEvent.click(screen.getByText('no'))
    expect(feedback).toHaveBeenCalledWith({ type: 'step_answer', nodeId: 'q', prompt: 'Which?', picked: 'no', attempts: 1 })
  })
  ...
  it('bug and match: a wrong pick is reported in words', () => {
    tracked(at({ type: 'bug', prompt: 'Tap', code: 'print("a")\nprint(b)', bugLine: 1, explain: 'Quotes.', picked: null, attempts: 0 }), ({ node, code }) => <BugNode node={node} code={code} />)
    fireEvent.click(screen.getByText('print("a")'))
    expect(feedback).toHaveBeenLastCalledWith(expect.objectContaining({ type: 'step_answer', picked: 'print("a")', attempts: 1 }))
    tracked(at({ type: 'match', prompt: 'Match', pairs: [{ left: 'print', right: 'speaks' }, { left: '"hi"', right: 'the words' }], matched: [], picked: null, attempts: 0 }), ({ node, code }) => <MatchNode node={node} code={code} />)
    fireEvent.click(screen.getAllByText('print')[0])
    fireEvent.click(screen.getAllByText('the words')[0])
    expect(feedback).toHaveBeenLastCalledWith(expect.objectContaining({ picked: 'print = the words' }))
  })
})

describe('stage step', () => {
  const stage = at({
    type: 'stage', prompt: 'Get the gem', scene: 'grid',
    config: { w: 3, h: 1, start: { x: 0, y: 0, dir: 'E' }, gems: [[2, 0]] }, goal: {},
    palette: [{ label: 'move', ops: ['move'] }, { label: 'turn left', ops: ['left'] }], solution: [0, 0], program: [], attempts: 0,
  })
  const feedback = jest.fn()
  const view = () => render(<Harness start={stage as never} View={({ node, code }) => <StageNode node={node as typeof stage} code={{ ...code, feedback } as CodeActions} />} />)
  // Each frame's timer is set by an effect, so advance in separate acts, one frame at a time.
  const play = (ms: number) => { for (let t = 0; t < ms; t += 600) act(() => { jest.advanceTimersByTime(600) }) }
  const build = (...labels: string[]) => labels.forEach((l) => fireEvent.click(within(screen.getByRole('group', { name: 'Blocks' })).getByText(l)))
  beforeEach(() => { jest.useFakeTimers(); feedback.mockClear() })
  afterEach(() => jest.useRealTimers())
  ...
  it('runs the program and resolves when the goal is met', () => {
    ...
    build('move', 'move')
    expect(within(screen.getByRole('list', { name: 'Your program' })).getAllByRole('button')).toHaveLength(2)
  ...
  it('lets a block come back out of the list', () => {
    ...
    expect(within(screen.getByRole('list', { name: 'Your program' })).getAllByRole('button')).toHaveLength(1)
  })
  ...
  it('tells the tutor about a miss, and after three shows a working program', () => {
    ...
    for (let round = 1; round <= 3; round++) {
      ...
      fireEvent.click(screen.getByText(/Run/))
      play(5000)
      expect(feedback).toHaveBeenLastCalledWith({ type: 'stage_result', nodeId: 'n', prompt: 'Get the gem', program: ['move'], attempts: round })
