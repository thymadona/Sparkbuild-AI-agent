...
jest.mock('@/lib/ratelimit', () => ({ checkRateLimit: async () => ({ allowed: true, hoursUntilReset: 0 }) }))
...
const board = (source: string, output?: string) => ({
  pages: [{ id: taskPageId(task), title: task.chip, nodeIds: output === undefined ? ['c1'] : ['c1', 'o1'] }],
  activePageId: taskPageId(task),
  focusId: null,
  nodes: {
    c1: { id: 'c1', parentId: null, createdBy: 'student', type: 'code', language: 'python', source, editable: true },
    ...(output === undefined ? {} : { o1: { id: 'o1', parentId: null, createdBy: 'system', type: 'output', forNodeId: 'c1', stdout: output, stderr: '', ok: true } }),
  },
})
...
const drain = async (res: Response) =>
  (await res.text()).split('\n\n').filter((l) => l.startsWith('data: ')).map((l) => JSON.parse(l.slice(6)) as { type: string; completedTaskIds?: string[] })
const run = (source: string, stdout: string) => ({ type: 'code_run_result', nodeId: 'c1', source, ok: true, stdout, stderr: '' })
...
const setup = async (source: string, output?: string, done = ['first-words', 'intro-3']) => {
  ...
  const project = await makeProject(user.id, { lessonId: lesson.id, lessonVersion: 3, files: { 'main.py': source }, board: board(source, output) })
  await setLessonProgress(project.id, done, new Date().toISOString())
  return { user, project }
}
const progressOf = async (id: string) => (await db.select({ ids: lessonProgress.completedTaskIds }).from(lessonProgress).where(eq(lessonProgress.projectId, id)))[0]?.ids

beforeEach(async () => {
  await resetDb()
  mockCreate.mockReset()
  // After a refused call the model gets one more go; by default it just talks.
  mockCreate.mockImplementation(async () => ({ async *[Symbol.asyncIterator]() { yield { choices: [{ delta: { content: 'Press Run.' } }] } } }))
})
...
describe('task_complete from the tutor', () => {
  it('records the task, streams task.complete and counts a day of activity', async () => {
    ...
    expect(events.find((e) => e.type === 'task.complete')?.completedTaskIds).toEqual(['first-words', 'intro-3', task.id])
    expect(await progressOf(project.id)).toEqual(['first-words', 'intro-3', task.id])
    expect(await db.select().from(activityDays).where(eq(activityDays.userId, user.id))).toHaveLength(1)
  })
  ...
  it('keeps an audit row of what the tutor saw and why it said yes', async () => {
    ...
    expect(rows[0]).toMatchObject({ taskId: task.id, reason: 'greets with an f-string', judgedBy: 'tutor', code: { 'main.py': DONE }, output: { 'main.py': 'Hi Moral\n' } })
  })
  ...
  it('is idempotent when the tutor calls it twice', async () => {
    const { project } = await setup(DONE)
    verdict([['task_complete', { taskId: task.id, reason: 'a' }], ['task_complete', { taskId: task.id, reason: 'b' }]])
    await drain(await post(project.id, run(DONE, 'Hi Moral\n')))
    expect(await progressOf(project.id)).toEqual(['first-words', 'intro-3', task.id])
  ...
  it('offers no task_complete while the concept steps are still showing', async () => {
    ...
    await db.update((await import('@/lib/db/schema')).projects).set({ board: { pages: [{ id: taskPageId(lesson.tasks[0]), title: 'x', nodeIds: [] }], activePageId: taskPageId(lesson.tasks[0]), focusId: null, nodes: {} } }).where(eq((await import('@/lib/db/schema')).projects.id, project.id))
    verdict([])
    await drain(await post(project.id, { type: 'student_message', text: 'hi' }))
...
describe('an edit after the last run', () => {
  it('is not accepted until the student runs it again', async () => {
    ...
    const project = await makeProject(user.id, { lessonId: lesson.id, lessonVersion: 3, files: { 'main.py': DONE }, board: b })
...
describe('a task with two programs', () => {
  ...
  const two = (secondRan: string | null) => ({
    pages: [{ id: page, title: fw.chip, nodeIds: ['a', 'oa', 'b', ...(secondRan === null ? [] : ['ob'])] }],
    activePageId: page,
    focusId: null,
    nodes: {
      a: { id: 'a', parentId: null, createdBy: 'student', type: 'code', language: 'python', source: 'print("hello Moral")', editable: true },
      oa: { id: 'oa', parentId: null, createdBy: 'system', type: 'output', forNodeId: 'a', stdout: 'hello Moral\n', stderr: '', ok: true, ran: 'print("hello Moral")' },
      b: { id: 'b', parentId: null, createdBy: 'student', type: 'code', language: 'python', file: 'line2.py', source: 'print("Hi")\nprint("see you")', editable: true },
      ...(secondRan === null ? {} : { ob: { id: 'ob', parentId: null, createdBy: 'system', type: 'output', forNodeId: 'b', stdout: secondRan, stderr: '', ok: true, ran: 'print("Hi")\nprint("see you")' } }),
    },
  })
  const start = async (b: object) => {
    const user = await makeUser()
    mockGetSessionUser.mockResolvedValue({ id: user.id, email: user.email, name: 'Mia' })
    const project = await makeProject(user.id, { lessonId: lesson.id, lessonVersion: 3, files: { 'main.py': 'print("hello Moral")' }, board: b })
  ...
  it('is refused while the second program has never run', async () => {
    ...
    await drain(await post(project.id, { type: 'code_run_result', nodeId: 'a', source: 'print("hello Moral")', ok: true, stdout: 'hello Moral\n', stderr: '' }))
  ...
  it('is accepted once both printed new words', async () => {
    ...
    complete('first-words')
    await drain(await post(project.id, { type: 'code_run_result', nodeId: 'b', source: 'print("Hi")\nprint("see you")', ok: true, stdout: 'Hi\nsee you\n', stderr: '' }))
  ...
  it('is refused when line 2 still says the starter words', async () => {
    const project = await start(two('Hi\nbye bye\n'))
    complete('first-words')
    await drain(await post(project.id, { type: 'code_run_result', nodeId: 'b', source: 'print("Hi")\nprint("bye bye")', ok: true, stdout: 'Hi\nbye bye\n', stderr: '' }))
    expect(await progressOf(project.id)).toEqual([])
  })
})
