...
async function boot() {
  py ??= loadPyodide().then((p) => { p.runPython(RUNTIME); return p })
  return py
}

async function prepare(files: Record<string, string>, inputs: string[]) {
  const p = await boot()
  for (const f of written) { try { p.FS.unlink(`${HOME}/${f}`) } catch { /* already gone */ } }
  ...
  p.setStdout({ write: (buf: Uint8Array) => { stdout += dec.decode(buf); return buf.length } })
...
export async function nodeTrace(source: string) {
  ...
  return JSON.parse(p.globals.get('_trace')('main.py') as string) as import('@/lib/board/schema').TraceStep[]
}
