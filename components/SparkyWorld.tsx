...
function draw(ctx: CanvasRenderingContext2D, s: WorldState, scene: Scene, asleep: boolean, t: number) {
  ctx.clearRect(0, 0, W, H)
  // Room
  ctx.fillStyle = '#2b2f4a'; ctx.fillRect(0, 0, W, 150)
  ctx.fillStyle = '#3d4270'; ctx.fillRect(0, 150, W, 50)
  ctx.fillStyle = '#4a5088'; ctx.fillRect(0, 150, W, 3)

  const cx = scene === 'vault' ? 100 : 160

  if (scene === 'vault') {
    ctx.fillStyle = '#6b7390'; ctx.fillRect(206, 46, 98, 108) // frame
    if (s.door === 'open') {
      ctx.fillStyle = '#0d1020'; ctx.fillRect(214, 54, 82, 92)
      ctx.fillStyle = `rgba(255, 214, 90, ${0.55 + 0.15 * Math.sin(t / 200)})` // glow
      ctx.fillRect(226, 108, 58, 38)
      ctx.fillStyle = '#ffd65a'; ctx.fillRect(238, 96, 34, 12)
      ctx.fillStyle = '#aab2c5'; ctx.fillRect(292, 54, 6, 92) // door swung aside
    } else {
      ctx.fillStyle = '#aab2c5'; ctx.fillRect(214, 54, 82, 92)
      ctx.fillStyle = '#8f98b0'; ctx.fillRect(220, 60, 70, 80)
      ctx.fillStyle = '#ffd65a'; ctx.fillRect(247, 92, 16, 16) // lock
      ctx.fillStyle = '#6b7390'; ctx.fillRect(251, 96, 8, 8)
    }
  }
  ...
  ctx.fillStyle = 'rgba(0,0,0,0.25)'; ctx.fillRect(cx - 28, 154, 56, 6) // shadow
  ctx.save(); ctx.translate(0, bob)
  ctx.fillStyle = '#1b1e33'; ctx.fillRect(cx - 32, 78, 64, 82) // outline
  ctx.fillStyle = color
  ctx.fillRect(cx - 20, 118, 40, 36) // body
  ctx.fillRect(cx - 28, 122, 8, 24); ctx.fillRect(cx + 20, 122, 8, 24) // arms
  ctx.fillRect(cx - 16, 154, 12, 6); ctx.fillRect(cx + 4, 154, 12, 6) // feet
  ctx.fillRect(cx - 24, 82, 48, 34) // head
  ctx.fillStyle = '#1b1e33'; ctx.fillRect(cx - 2, 68, 4, 14) // antenna
  ctx.fillStyle = s.alarm ? '#ff3b57' : '#ffd65a'; ctx.fillRect(cx - 6, 62, 12, 8)
  ctx.fillStyle = '#ffffff'
  if (asleep) {
    ctx.fillRect(cx - 16, 98, 10, 3); ctx.fillRect(cx + 6, 98, 10, 3) // closed eyes
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 14px monospace'
    ctx.fillText('z', cx + 30, 76 - ((t / 400) % 3) * 4); ctx.fillText('Z', cx + 40, 62 - ((t / 400) % 3) * 4)
  } else {
    const blink = t % 3000 < 120
    ctx.fillRect(cx - 16, 92, 10, blink ? 3 : 12); ctx.fillRect(cx + 6, 92, 10, blink ? 3 : 12)
    if (!blink) { ctx.fillStyle = '#1b1e33'; ctx.fillRect(cx - 12, 96, 5, 6); ctx.fillRect(cx + 10, 96, 5, 6) }
    ctx.fillStyle = '#1b1e33'; ctx.fillRect(cx - 8, 108, 16, 3) // mouth
  }
  ctx.restore()

  if (s.alarm) {
    ctx.fillStyle = `rgba(255, 40, 60, ${0.14 + 0.14 * Math.sin(t / 150)})`; ctx.fillRect(0, 0, W, H)
    ctx.fillStyle = '#ff3b57'; ctx.font = 'bold 16px monospace'; ctx.fillText('ALARM!', 236, 30)
  }
  ...
  if (s.say !== null) {
    ...
    ctx.fillStyle = '#ffffff'; ctx.fillRect(x, 4, width, height)
    ctx.fillRect(cx - 4, 4 + height, 8, 4); ctx.fillRect(cx - 2, 4 + height + 4, 4, 3) // tail
