  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-4">
        ...
        {allDone && (
          <div className="rounded-xl bg-teal-50 dark:bg-teal-900/20 px-4 py-4 text-center animate-pop-in">
            <p className="text-sm text-teal-700 dark:text-teal-300">🎉 All done! You completed every task in this lesson.</p>
          </div>
        )}
        ...
        <div className="flex flex-col gap-1.5">
          {lesson.tasks.map((task, i) => {
            ...
            return (
              ...
              >
                {/* Status indicator */}
                <span className={`shrink-0 flex h-5 w-5 items-center justify-center rounded-full text-xs font-medium ${
                  isDone ? 'bg-green-100 dark:bg-green-950 text-green-600 dark:text-green-500' : isActive ? 'bg-brand-100 dark:bg-brand-900 text-brand-700 dark:text-brand-300' : 'bg-surface-700 text-fg-muted'
                }`}>
                  {isDone ? (
        ...
        {/* Help chips */}
        <div>
          <p className="text-xs font-medium uppercase tracking-widest text-fg-muted mb-1.5">
            Help
          </p>
          <div className="flex flex-col gap-1.5">
            {HELP_CHIPS.map((chip) => (
              <button
                ...
                className="flex items-center gap-2 rounded-md px-3 py-2 text-left text-xs text-fg-muted hover:bg-surface-700 hover:text-fg-secondary transition-colors"
              >
      ...
      {activeTask && !allDone && (
        <div className="shrink-0 border-t border-surface-600 p-3">
          <p className="text-xs text-fg-muted mb-2 text-center">
            Finished <span className="text-fg-secondary">{activeTask.chip}</span>?
          </p>
          <button
