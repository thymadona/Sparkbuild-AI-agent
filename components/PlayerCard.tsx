  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-fg-muted">Your level</p>
        ...
        >
          ...
          <span className="font-display text-sm font-bold text-fg-primary">{streak} {streak === 1 ? 'day' : 'days'}</span>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        {badges.length ? (
          <ul className="mt-2 flex flex-wrap gap-2">
            {badges.map((badge) => (
              <li key={badge} className="rounded-full bg-tint-sand px-3 py-1 text-sm font-bold text-fg-primary">
                🏅 {badge}
              </li>
            ))}
          </ul>
